...
const savingHopePetPageUrl = "https://www.petfinder.com/search/dogs-for-adoption/?include_transportable=0&shelter_id%5B0%5D=TX2337&sort%5B0%5D=recently_added";
...
(async () => {
  ...
  await page.goto(savingHopePetPageUrl);
  ...
  FileUtility.writeFile(`${defaultOutputDirectoryName}\\petLinkList.txt`, petLinks.join("\r\n"));
...
async function GetAllPetLinks(index:number){
  ...
  try{
    ...
    await page.goto(`${savingHopePetPageUrl}&page=${index}`);
    //await page.waitForNetworkIdle();
    ...
    linkList.forEach((link)=>{
      if(link != null && !petLinks.includes(link)){
        petLinks.push(link);
      }
    });

  } catch(ex) {
    console.error(`ERROR: Could not query page ${index}. Exception:${ex}`);
  } finally{
    page.close();
  }
}
...
async function BuildPetFlyer(url:string){
  if(browser == undefined){
    console.error("ERROR: Could not query page. Browser is undefined.");
    return;
  }
  ...
  try{
    ...
    FileUtility.writeFile(`${defaultOutputDirectoryName}\\${petNameData.trim()}.html`, template);
